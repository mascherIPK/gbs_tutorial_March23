...
<2> Find the node on which your job is running. Replace mascher with your username.
...
<2> Close the file (switch the plotting device off). Don't forget to call dev.off(). Otherwise, the PDF file will be empty or invalid.
...
. To close R, type `quit()`. To close the tmux session and SLURM, type `exit` twice. This will finish the tmux session, if you just want to detach, use `tmux detach`.
