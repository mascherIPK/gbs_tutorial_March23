...
= Genotyping-by-sequencing of biparental populations to map traits
...
. Obtain a Kerberos ticket, allocate an interactive session with https://slurm.schedmd.com/salloc.html[salloc].
+
[source,sh]
----
kinit
auks -a
salloc --auks=yes -p cpu -c 4 --mem 10G #change cpu to queue for students
hostname // <1>
----
<1> Check that you are on one of the SLURM compute nodes.
...
. Create symbolic links to the GBS reads.
+
[source,sh]
----
pwd // <1>

ln -s /filer-dg/agruppen/dg6/mascher/DG/mxb_course_221201/data/MxB_92_RILs_phenotypes.csv .
----
<1> Check that you in your working directory (`/filer/projekte/mxb_tutorial/mascher/gbs`)

== Read mapping ==

. We will need the tools https://github.com/lh3/bwa[BWA] and http://www.htslib.org/doc/samtools.html[samtools] for read mapping and processing of alignment records, respectively.
+
[source,sh]
----
...
. Open a new SSH session on the slurm-login node. Start https://www.hamvocke.com/blog/a-quick-and-easy-guide-to-tmux/[tmux] to keep your commands running in the background. Then start an interactive SLURM session
+
[source,sh]
----
tmux bash
...
. Run the mapping for all samples.
+
[source,sh]
----
ref='/filer-dg/agruppen/dg6/mascher/DG/mxb_course_221201/data/reference/Hordeum_vulgare_MorexV3_assembly.fasta'
...
. If you forget to start `bwa` inside a `tmux` session, there is no way you prevent your job from aborting when you exit when you shutdown your laptop. Tough luck.

. Calculate mapping rates for all samples.
+
[source,sh]
----
...
join tmp1 tmp2 | awk '{print $0,$3/$2*100}' | sort -nk 4 | column -t > mapping_stats.txt // <2><3>

rm -f tmp1 tmp2
----
<1> To combine two lists with https://linux.die.net/man/1/join[join], both lists need to be sorted on the common ID column.
<2> We use https://www.tutorialspoint.com/awk/index.htm[AWK] to calculate the proportion of mapped reads.
<3> https://linux.die.net/man/1/column[column] is used to align columns.
...
<3> We ignore insertions and deletions (`--skip-indels`), consider only SNPs with a quality score no smaller than 20 (`--min-MQ 20`) and add allelic depth (`--annotate AD`) information for all genotype calls.
...
----
<1> We keep homozygous genotype calls if they have at least two supporting reads; heterozygous calls are accepted if they are supported by four reads. SNPs with a minor allele frequency below 20 % or fewer than 90 % present calls or fewer than 90 % homozyous calls are discarded.

. Change the column names to match the row names in the phenotype table.
+
[source,sh]
----
less MxB_92_RILs_phenotypes.csv
...
== Import the data into R/qtl

. Start R. We use version 3.5.1.
+
[source,sh]
----
module load R/3.5.1
...
[source,r]
----
.libPaths(c("/filer-dg/agruppen/seq_shared/mascher/Rlib/3.5.1")) // <1>

library("qtl") // <2>
library("ASMap") // <3> 
library("utl") // <4>
----
<1> Set the path where the libraries are located.
...
ids <- paste(read.table("new_sample_names.txt", head=F)[, 1])
genfile <- 'bcftools_SNP_calling_geno.txt'
...
----
<1> This function write a text file in "csvs" format to disk. The output filename is `genfile`. 

. The conversion function doesn't use genomic coordinates, so markers are ordered correctly, but equidistant.
+
[source,r]
----
...
----
<1> The `sed` command retrieves the genomic coordinates from the first line and inserts them as a new line after line 2.
...
If there are issues with the data, geneitc the map will be odd.
...
. Thin the set of markers to keep one per megabase.
+
[source,sh]
----
...
merge(a, b, by=0) -> m // <1>
sapply(split(m, m$chr.x), function(i) with(i, cor(pos.x, pos.y, method='s'))) -> cor // <2>
----
...
. Run the "QTL" scan for the first phenotypes, lateral spikelet fertility, commonly known as row type.
